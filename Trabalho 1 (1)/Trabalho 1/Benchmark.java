...
public class Benchmark {
    ...
    // ===== Config =====
    static final int[] TAMANHOS_ALEATORIO = {100, 1_000, 10_000, 100_000};
    static final int    TAMANHO_ORDENADOS = 100_000;
    static final int REPETICOES = 10;

    // ===== Pares (nome + função) =====
    static final class AlgComp {
        final String nome; final Consumer<Integer[]> f;
        AlgComp(String n, Consumer<Integer[]> f){ this.nome=n; this.f=f; }
    }
    static final class AlgLin  {
        final String nome; final Consumer<int[]> f;
        AlgLin(String n, Consumer<int[]> f){ this.nome=n; this.f=f; }
    }
    ...
    static Path ds(int n, String ordem){ return Paths.get("datasets","n"+n+"_"+ordem+".csv"); }
    ...
    public static void main(String[] args) throws Exception {
        // comparativos (Comparable[])
        AlgComp[] comp = new AlgComp[]{
                new AlgComp("bubble",    ordenadores::bubbleSort),
                new AlgComp("insertion", ordenadores::insertionSort),
                new AlgComp("selection", ordenadores::selectionSort),
                new AlgComp("shell",     ordenadores::shellSort),
        ...
        // acumulador de médias: chave "algo,ordem,n" -> [soma, cont]
        Map<String, long[]> medias = new HashMap<>();

        // ----- A) ALEATÓRIO nos tamanhos 100..100k -----
        for (int n : TAMANHOS_ALEATORIO) {
            String ordem = "aleatorio";
            Path arq = ds(n, ordem);
            if (!Files.exists(arq)) { System.err.println("Dataset ausente: "+arq.toAbsolutePath()); continue; }

            int[] baseInt = carregarCSVInt(arq);
            Integer[] baseInteger = toInteger(baseInt);
            int valorMaximo = max(baseInt);
            int baseRadix = 10, baldes = 16;
        ...
        // ----- B) CRESCENTE e DECRESCENTE só em n = 100_000 -----
        for (String ordem : new String[]{"crescente","decrescente"}) {
            int n = TAMANHO_ORDENADOS;
            Path arq = ds(n, ordem);
            if (!Files.exists(arq)) { System.err.println("Dataset ausente: "+arq.toAbsolutePath()); continue; }
        ...
        // ----- Médias -----
        Path mediasCsv = dirSummary.resolve("medias_por_algo.csv");
        ...
        System.out.println("OK! RAW em results/raw/*.csv  |  Médias: " + mediasCsv.toAbsolutePath());
    }
}
